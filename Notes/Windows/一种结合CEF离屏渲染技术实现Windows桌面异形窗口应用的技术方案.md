...
RGBA：表示色彩空间的一种编码格式，RGBA分别代表Red（红色） Green（绿色） Blue（蓝色）和 Alpha(不透明度），一个RGBA值表示了一个像素的可视属性。
...
1. 创建（CreateWindowEx）或更改窗口（SetWindowLongPtr）使其扩展属性包含WS\*EX\*LAYERED；
...
  *In*    HWND hwnd,
  *In*opt_  HDC hdcDst,
  *In*opt_  POINT *pptDst,
  *In*opt_  SIZE *psize,
  *In*opt_  HDC hdcSrc,
  *In*opt_  POINT *pptSrc,
  *In*      COLORREF crKey,
  *In*opt_  BLENDFUNCTION *pblend,
  *In*      DWORD dwFlags
);
```
