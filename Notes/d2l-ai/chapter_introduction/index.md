...
我们试着用一台计算机和一个代码编辑器编写代码，如 :numref:`fig*wake*word`中所示。
...
如 :numref:`fig*ml*loop`所示，训练过程通常包含如下步骤：
...
整个监督学习过程如 :numref:`fig*supervised*learning` 所示。
...
举个例子，假设后院有一个如 :numref:`fig*death*cap` 所示的蘑菇。
...
假设这个分类器输出 :numref:`fig*death*cap` 包含死帽蕈的概率是0.2。
...
事实上，谨慎是有道理的， :numref:`fig*death*cap`中的蘑菇实际上是一个死帽蕈。
...
比如， :numref:`fig*deeplearning*amazon` 是亚马逊基于个性化算法推荐的深度学习书籍，成功地捕捉了作者的喜好。
...
对于监督学习，从环境中收集数据的过程类似于 :numref:`fig*data*collection`。
...
而高斯分布是由[卡尔•弗里德里希•高斯（1777-1855）](https://en.wikipedia.org/wiki/Carl*Friedrich*Gauss)发现的，
...
机器学习的第二个影响来自[克劳德·香农(1916--2001)](https://en.wikipedia.org/wiki/Claude*Shannon)的信息论和[艾伦·图灵（1912-1954）](https://en.wikipedia.org/wiki/Alan*Turing)的计算理论。
图灵在他著名的论文《计算机器与智能》 :cite:`Turing.1950` 中提出了“机器能思考吗？”的问题。
在他所描述的图灵测试中，如果人类评估者很难根据文本互动区分机器和人类的回答，那么机器就可以被认为是“智能的”。

另一个影响可以在神经科学和心理学中找到。
其中，最古老的算法之一是[唐纳德·赫布 (1904--1985)](https://en.wikipedia.org/wiki/Donald*O.*Hebb)开创性的著作《行为的组织》 :cite:`Hebb.Hebb.1949` 。
...
这一点在 :numref:`tab*intro*decade` 中得到了说明。
...
1. 你还可以在哪里应用端到端的训练方法，比如 :numref:`fig*ml*loop` 、物理、工程和计量经济学？
