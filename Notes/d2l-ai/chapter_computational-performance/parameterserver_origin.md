...
Consider a real 4-way GPU server as described in :numref:`fig*bw*hierarchy`. If it is particularly well connected, it might have a 100 GbE network card. More typical numbers are in the 1--10 GbE range with an effective bandwidth of 100 MB/s to 1 GB/s.
...
Synchronization across devices is tricky. After all, different machines running training code will have subtly different speed. Hence we need to *synchronize* them if we want to use synchronous distributed optimization. :numref:`fig*ps*multimachine` illustrates how distributed parallel training occurs.
...
Matching both numbers yields constant scaling regardless of how many workers we are dealing with. In practice we use the *same* machines both as workers and as servers. :numref:`fig*ps*multips` illustrates the design (see also :cite:`Li.Andersen.Park.ea.2014` for details). 
