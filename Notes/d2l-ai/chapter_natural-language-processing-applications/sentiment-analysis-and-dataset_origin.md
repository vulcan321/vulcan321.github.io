...
in :numref:`sec*machine*translation`.
...
```{.python .input}
# @tab pytorch
train*iter = d2l.load*array((train*features, torch.tensor(train*data[1])), 64)
...
Last, we wrap up the above steps into the `load*data*imdb` function.
