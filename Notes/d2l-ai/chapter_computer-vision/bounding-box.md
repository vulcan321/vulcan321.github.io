...
前面的章节（例如 :numref:`sec*alexnet`— :numref:`sec*googlenet`）介绍了各种图像分类模型。
...
在这里，我们[**定义在这两种表示法之间进行转换的函数**]：`box*corner*to*center`从两角表示法转换为中心宽度表示法，而`box*center*to*corner`反之亦然。
...
1. 为什么`box*corner*to*center`和`box*center*to*corner`的输入参数的最内层维度总是4？
