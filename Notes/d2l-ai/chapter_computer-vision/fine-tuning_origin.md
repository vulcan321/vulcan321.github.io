...
First, we define a training function `train*fine*tuning` that uses fine-tuning so it can be called multiple times.
...
2. Further adjust hyperparameters of `finetune*net` and `scratch*net` in the comparative experiment. Do they still differ in accuracy?
