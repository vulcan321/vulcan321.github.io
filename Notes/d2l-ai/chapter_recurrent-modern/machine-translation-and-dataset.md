...
与 :numref:`sec*language*model`中的语料库
是单一语言的语言模型问题存在不同，
...
与 :numref:`sec*language*model`中的字符级词元化不同，
在机器翻译中，我们更喜欢单词级词元化
（最先进的模型可能使用更高级的词元化技术）。
下面的`tokenize*nmt`函数对前`num*examples`个文本序列对进行词元，
...
# @tab all
# @save
def show*list*len*pair*hist(legend, xlabel, ylabel, xlist, ylist):
    """绘制列表长度对的直方图"""
    d2l.set_figsize()
...
show*list*len*pair*hist(['source', 'target'], '# tokens per sequence',
                        'count', source, target);
```
...
这个固定长度是由 :numref:`sec*language*model`中的
...
def truncate*pad(line, num*steps, padding_token):
    ...
    return line + [padding*token] * (num*steps - len(line))  # 填充
...
最后，我们定义`load*data*nmt`函数来返回数据迭代器，
以及源语言和目标语言的两种词表。
...
for X, X*valid*len, Y, Y*valid*len in train_iter:
    ...
    print('X的有效长度:', X*valid*len)
    print('Y:', d2l.astype(Y, d2l.int32))
    print('Y的有效长度:', Y*valid*len)
    break
```
...
1. 在`load*data*nmt`函数中尝试不同的`num_examples`参数值。这对源语言和目标语言的词表大小有何影响？
