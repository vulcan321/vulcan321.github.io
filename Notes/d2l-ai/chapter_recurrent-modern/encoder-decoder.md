...
正如我们在 :numref:`sec*machine*translation`中所讨论的，
机器翻译是序列转换模型的一个核心问题，
其输入和输出都是长度可变的序列。
为了处理这种类型的输入和输出，
我们可以设计一个包含两个主要组件的架构：
...
如 :numref:`fig*encoder*decoder` 所示。
...
这在 :numref:`subsec*mt*data_loading`中进行了解释。
为了逐个地生成长度可变的词元序列，
解码器在每个时间步都会将输入
...
class EncoderDecoder(tf.keras.Model):
    ...
    def call(self, enc*X, dec*X, *args, **kwargs):
        enc*outputs = self.encoder(enc*X, *args, **kwargs)
        dec*state = self.decoder.init*state(enc_outputs, *args)
        return self.decoder(dec*X, dec*state, **kwargs)
```
