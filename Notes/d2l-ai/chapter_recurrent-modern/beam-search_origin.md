...
$$y*{t'} = \operatorname*{argmax}*{y \in \mathcal{Y}} P(y \mid y*1, \ldots, y*{t'-1}, \mathbf{c}),$$
...
with the maximum 
$\prod*{t'=1}^{T'} P(y*{t'} \mid y*1, \ldots, y*{t'-1}, \mathbf{c})$,
which is
...
have changed from "A" and "B" in :numref:`fig*s2s-prob1` to "A" and "C" in :numref:`fig*s2s-prob2`, 
...
Therefore, the conditional probability of generating each token at time step 4 in :numref:`fig*s2s-prob2` is also different from that in :numref:`fig*s2s-prob1`. 
As a result, 
...
At time step 1, 
suppose that the tokens with the highest conditional probabilities $P(y*1 \mid \mathbf{c})$ are $A$ and $C$. At time step 2, for all $y*2 \in \mathcal{Y},$ we compute 

$$\begin{aligned}P(A, y*2 \mid \mathbf{c}) = P(A \mid \mathbf{c})P(y*2 \mid A, \mathbf{c}),\\ P(C, y*2 \mid \mathbf{c}) = P(C \mid \mathbf{c})P(y*2 \mid C, \mathbf{c}),\end{aligned}$$  
...
$$\begin{aligned}P(A, B, y*3 \mid \mathbf{c}) = P(A, B \mid \mathbf{c})P(y*3 \mid A, B, \mathbf{c}),\\P(C, E, y*3 \mid \mathbf{c}) = P(C, E \mid \mathbf{c})P(y*3 \mid C, E, \mathbf{c}),\end{aligned}$$ 
...
$$ \frac{1}{L^\alpha} \log P(y*1, \ldots, y*{L}) = \frac{1}{L^\alpha} \sum*{t'=1}^L \log P(y*{t'} \mid y*1, \ldots, y*{t'-1}, \mathbf{c}),$$
...
1. We used language modeling for generating text following  user-provided prefixes in :numref:`sec*rnn*scratch`. Which kind of search strategy does it use? Can you improve it?
