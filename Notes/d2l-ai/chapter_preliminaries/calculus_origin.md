...
As shown in :numref:`fig*circle*area`,
...
In fact, the method of exhaustion is where *integral calculus* (will be described in :numref:`sec*integral*calculus`) originates from.
...
As discussed in :numref:`subsec*norms*and_objectives`,
...
In the following,
the `use*svg*display` function specifies the `matplotlib` package to output the svg figures for sharper images.
...
so later they can be directly invoked (e.g., `d2l.use*svg*display()`) without being redefined.
...
Let $y = f(x*1, x*2, \ldots, x*n)$ be a function with $n$ variables. The *partial derivative* of $y$ with respect to its $i^\mathrm{th}$  parameter $x*i$ is

$$ \frac{\partial y}{\partial x*i} = \lim*{h \rightarrow 0} \frac{f(x*1, \ldots, x*{i-1}, x*i+h, x*{i+1}, \ldots, x*n) - f(x*1, \ldots, x*i, \ldots, x*n)}{h}.$$


To calculate $\frac{\partial y}{\partial x*i}$, we can simply treat $x*1, \ldots, x*{i-1}, x*{i+1}, \ldots, x*n$ as constants and calculate the derivative of $y$ with respect to $x*i$.
...
Suppose that the input of function $f: \mathbb{R}^n \rightarrow \mathbb{R}$ is an $n$-dimensional vector $\mathbf{x} = [x*1, x*2, \ldots, x_n]^\top$ and the output is a scalar. The gradient of the function $f(\mathbf{x})$ with respect to $\mathbf{x}$ is a vector of $n$ partial derivatives:
...
Similarly, for any matrix $\mathbf{X}$, we have $\nabla*{\mathbf{X}} \|\mathbf{X} \|*F^2 = 2\mathbf{X}$. As we will see later, gradients are useful for designing optimization algorithms in deep learning.
...
$u*1, u*2, \ldots, u*m$, where each differentiable function $u*i$
has variables $x*1, x*2, \ldots, x_n$.
Note that $y$ is a function of $x*1, x*2, \ldots, x_n$.
Then the chain rule gives

$$\frac{dy}{dx*i} = \frac{dy}{du*1} \frac{du*1}{dx*i} + \frac{dy}{du*2} \frac{du*2}{dx*i} + \cdots + \frac{dy}{du*m} \frac{du*m}{dx*i}$$
...
1. Find the gradient of the function $f(\mathbf{x}) = 3x*1^2 + 5e^{x*2}$.
