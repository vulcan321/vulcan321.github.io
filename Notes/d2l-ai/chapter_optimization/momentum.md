...
为了保持记法简单，在这里我们使用$\mathbf{h}*{i, t-1} = \partial*{\mathbf{w}} f(\mathbf{x}*i, \mathbf{w}*{t-1})$作为样本$i$的随机梯度下降，使用时间$t-1$时更新的权重$t-1$。
...
回想我们在 :numref:`sec*gd`中使用了$f(\mathbf{x}) = x*1^2 + 2 x_2^2$，即中度扭曲的椭球目标。
...
从构造来看，$x*2$方向的梯度比水平$x*1$方向的梯度大得多，变化也快得多。
因此，我们陷入两难：如果选择较小的学习率，我们会确保解不会在$x*2$方向发散，但要承受在$x*1$方向的缓慢收敛。相反，如果学习率较高，我们在$x*1$方向上进展很快，但在$x*2$方向将会发散。
...
使用$\mathbf{v}*t$而不是梯度$\mathbf{g}*t$可以生成以下更新等式：
...
唯一的变化是，在这种情况下，我们将梯度$\mathbf{g}*{t, t-1}$替换为$\mathbf{g}*t$。
为了方便起见，我们在时间$t=0$初始化$\mathbf{v}_0 = 0$。

## # 有效样本权重

回想一下$\mathbf{v}*t = \sum*{\tau = 0}^{t-1} \beta^{\tau} \mathbf{g}_{t-\tau, t-\tau-1}$。
...
d2l.train*concise*ch11(trainer, {'learning*rate': 0.005, 'momentum': 0.9}, data*iter)
```

# # 理论分析

$f(x) = 0.1 x*1^2 + 2 x*2^2$的2D示例似乎相当牵强。
...
每$|1 - \eta \lambda| < 1$时，这种优化以指数速度收敛，因为在$t$步之后我们可以得到$x*t = (1 - \eta \lambda)^t x*0$。
...
在$t$步之后，最初的值$[v*0, x*0]$变为$\mathbf{R}(\beta, \eta, \lambda)^t [v*0, x*0]$。
...
1. 试试梯度下降和动量法来解决一个二次问题，其中有多个特征值，即$f(x) = \frac{1}{2} \sum*i \lambda*i x*i^2$，例如$\lambda*i = 2^{-i}$。绘制出$x$的值在初始化$x_i = 1$时如何下降。
