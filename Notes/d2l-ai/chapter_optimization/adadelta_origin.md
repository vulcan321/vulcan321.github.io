...
The difference to :numref:`sec*rmsprop` is that we perform updates with the rescaled gradient $\mathbf{g}*t'$, i.e.,
...
$$\begin{aligned}
    \mathbf{g}*t' & = \frac{\sqrt{\Delta\mathbf{x}*{t-1} + \epsilon}}{\sqrt{{\mathbf{s}*t + \epsilon}}} \odot \mathbf{g}*t, \\
\end{aligned}$$

where $\Delta \mathbf{x}*{t-1}$ is the leaky average of the squared rescaled gradients $\mathbf{g}*t'$. We initialize $\Delta \mathbf{x}*{0}$ to be $0$ and update it at each step with $\mathbf{g}*t'$, i.e.,
...
Adadelta needs to maintain two state variables for each variable, $\mathbf{s}*t$ and $\Delta\mathbf{x}*t$. This yields the following implementation.
...
def init*adadelta*states(feature_dim):
    s*w, s*b = d2l.zeros((feature_dim, 1)), d2l.zeros(1)
    delta*w, delta*b = d2l.zeros((feature_dim, 1)), d2l.zeros(1)
    return ((s*w, delta*w), (s*b, delta*b))
...
def init*adadelta*states(feature_dim):
    ...
    delta*w = tf.Variable(d2l.zeros((feature*dim, 1)))
...
d2l.train*ch11(adadelta, init*adadelta*states(feature*dim),
               {'rho': 0.9}, data*iter, feature*dim);
```
...
```{.python .input}
d2l.train*concise*ch11('adadelta', {'rho': 0.9}, data_iter)
```

```{.python .input}
# @tab pytorch
trainer = torch.optim.Adadelta
d2l.train*concise*ch11(trainer, {'rho': 0.9}, data_iter)
```
...
d2l.train*concise*ch11(trainer, {'learning*rate':5.0, 'rho': 0.9}, data*iter)
```
