...
问题在于，Adagrad算法将梯度$\mathbf{g}*t$的平方累加成状态矢量$\mathbf{s}*t = \mathbf{s}*{t-1} + \mathbf{g}*t^2$。
...
另一种方法是按动量法中的方式使用泄漏平均值，即$\mathbf{s}*t \leftarrow \gamma \mathbf{s}*{t-1} + (1-\gamma) \mathbf{g}_t^2$，其中参数$\gamma > 0$。
...
和之前一样，我们使用二次函数$f(\mathbf{x})=0.1x*1^2+2x*2^2$来观察RMSProp算法的轨迹。
...
1. 旋转优化问题以最小化$f(\mathbf{x}) = 0.1 (x*1 + x*2)^2 + 2 (x*1 - x*2)^2$。收敛会发生什么？
