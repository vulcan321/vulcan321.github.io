...
![Introduction](img/B04929*02*01.jpg)
...
5.  将以下代码行添加到`Enemy.h`中：
    ...
    class CEnemy {
    protected:
      int m*ihealth,m*iarmourValue;
    public:
      CEnemy(int ihealth, int iarmourValue) : m*ihealth(ihealth), m*iarmourValue(iarmourValue) {}
...
6.  将以下代码行添加到`Dragon.h`：
    ...
    public:
      CDragon(int m*ihealth, int m*iarmourValue) : CEnemy(m*ihealth, m*iarmourValue)
      {
      }
      int TotalHP()
      {
        cout << "Dragon's ";
        return m*ihealth*2+3*m*iarmourValue;
      }
    };
...
7.  将以下代码行添加到`Soldier.h`中：
    ...
    public:
      CSoldier(int m*ihealth, int m*iarmourValue) : CEnemy(m*ihealth, m*iarmourValue) {}
      int TotalHP()
      {
        cout << "Soldier's ";
        return m*ihealth+m*iarmourValue;
      }
    };
...
6.  将以下行代码添加到`vector3.h`：
    ...
    public:
      ...
      CVector3(float *fx, float *fy, float _fz);
      ...
      {
        return (sqrt(*kr.x * *kr.x + *kr.y * *kr.y + *kr.z * *kr.z));
      }
        ...
        const CVector3& _krB)
      {
        return
          (
          CVector3(*krA.y * *krB.z - *krA.z * *krB.y,
          *krA.z * *krB.x - *krA.x * *krB.z,
          *krA.x * *krB.y - *krA.y * *krB.x)
          );
      }
      ...
      {
        float fdx = *krA.x - *krB.x;
        float fdy = *krA.y - *krB.y;
        float fdz = *krA.z - *krB.z;
    ...
    inline CVector3 operator *(float *f, const CVector3& *kr)
    {
      return (CVector3(*f * *kr.x, *f * *kr.y, *f * *kr.z));
    }
...
7.  将以下行代码添加到`vector3.cpp`：
    ...
    CVector3::CVector3(float *fx, float *fy, float _fz)
    : x(_fx)
    , y(_fy)
    , z(_fz)
    {
    ...
    CVector3::operator +(const CVector3& _kr) const
    {
      return (CVector3(x + *kr.x, y + *kr.y, z + _kr.z));
    }
...
5.  将以下代码行添加到`Stack.h`：
    ...
    {
      if (elements.empty()) {
        throw out*of*range("Stack<>::pop(): empty stack");
      }
      // remove last element
      elements.pop_back();
    }
