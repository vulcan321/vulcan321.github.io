...
![Controlling the panning](img/9099OT*03*01.jpg)
...
class AudioManager {
 ...
 private:
  ...
  enum Category { CATEGORY*SFX, CATEGORY*SONG, CATEGORY_COUNT };
...
AudioManager::AudioManager() : currentSong(0), fade(FADE_NONE) {
 ...
 modes[CATEGORY*SFX] = FMOD*DEFAULT;
 modes[CATEGORY*SONG] = FMOD*DEFAULT | FMOD_CREATESTREAM |
 FMOD*LOOP*NORMAL;
...
void AudioManager::PlaySFX(const std::string& path,
  ...
  FMOD::Channel* channel;
  system->playSound(FMOD*CHANNEL*FREE, sound->second,
                    true, &channel);
...
*   如果没有正在播放的歌曲，我们可以立即开始播放新歌，将其初始音量设置为零，并将管理器设置为淡入状态。 歌曲还必须添加到正确的通道组：
    ...
    void AudioManager::PlaySong(const std::string& path) {
      ...
      system->playSound(FMOD*CHANNEL*FREE, 
