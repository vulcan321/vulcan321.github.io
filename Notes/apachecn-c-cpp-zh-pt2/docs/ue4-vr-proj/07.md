...
5.  打开新的`.uproject`文件。 您将看到您刚刚删除的`Intermediate`和`Saved`目录已为新项目重新生成。项目应打开到我们在上一章中设置的默认映射(`LV*Soul*Slum_Mobile`)。
6.  点击工具栏的构建按钮以重建其照明。
...
1.  打开`WBP*CompanionIndicator`并从设计面板或层次选项卡中选择`txt*StateIndicator`。
...
1.  选择*`BP*CompanionCharacter`*class‘_`Companion State`变量，并在其详细信息中将其 Private 标志设置为 true。
...
1.  打开`BP*VRPawn`，在其组件列表中找到`Hand*L`。
...
We could also simply have dragged a connector from Set Tracking Origin output to the GetHand*L call, but it's a better practice to keep unrelated operations on separate execution lines so it's easier to see what really belongs together. By putting Set Tracking Origin on one sequence output, and the GetHand*L call on another, we're making it clear to the reader that these are two separate jobs being done.
...
17.  将`MI*WidgetInteractionBeam`分配给`BP*VRPawn`上的`InteractionBeam`静态网格组件。
...
20.  将`MI*WidgetInteractionTarget`指定给我们在`BP*VRPawn`上创建的`InteractionBeamTarget`球体。

最后，我们需要将其位置设置为交互组件的影响位置。

21.  在`BP*VRPawn`玩家的事件图中，找到`Event Tick`，并在`Event Tick`和`UpdateTeleport*Implementation`折叠图之间创建一个序列节点。
