...
![Introducing layout](img/5022*04*01.jpg)
...
感兴趣的读者可能想知道如何在代码中为`Width`指定`"Auto"`XAML 值，因为这是一个`double`值。 这是通过包括`<limits>`，然后使用表达式`std::numeric*limits<double>::quiet*NaN()`来实现的。 同样，要指定一个无限值，请使用`std::numeric_limits<double>::infinity()`。
...
void MainPage::OnPointerPressed(Platform::Object^ sender,
  ...
  *lastPoint.X -= (float)Canvas::GetLeft(*element);
  *lastPoint.Y -= (float)Canvas::GetTop(*element);
...
![Adding children to a panel dynamically](img/5022*04*05.jpg)
...
![VariableSizedWrapGrid](img/5022*04*06.jpg)
...
![Working with elements and controls](img/5022*04*07.jpg)
...
![Content controls](img/5022*04*08.jpg)
...
![Content controls](img/5022*04*09.jpg)
...
![Content controls](img/5022*04*10.jpg)
...
![Using custom fonts](img/5022*04*16.jpg)
...
![Using custom fonts](img/5022*04*15.jpg)
...
*   `InputScope` – indicates what kind of virtual keyboard should pop up on a touch-based device that is not using a physical keyboard. This can help with text entering. Values (from the `InputScopeNameValue` enumeration) include: `Url`, `Number`, `EmailSmtpAddress` (e-mail address), and others. Here is a keyboard screenshot for `InputScope` of `Number`:
    ...
    ![TextBox](img/5022*04*22.jpg)

这是`Url`的`InputScope`的键盘：

![TextBox](img/5022*04*23.jpg)

此命令适用于`EmailSmtpAddress`的`InputScope`：

![TextBox](img/5022*04*24.jpg)
...
![The Stretch property](img/5022*04*18.jpg)
...
![The SemanticZoom control](img/5022*04*20.jpg)

这是缩小的视图：

![The SemanticZoom control](img/5022*04*19.jpg)
