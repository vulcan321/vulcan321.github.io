...
![The Start (Home) screen](img/5022*01*01.jpg)
...
![The Charms bar](img/5022*01*02.jpg)
...
![The Windows Runtime](img/5022*01*03.jpg)
...
![Creating your first Store application](img/5022*01*04.jpg)
...
![Creating your first Store application](img/5022*01*05.jpg)

两个`TextBox`控件(名为`*number1`和`*number2`)用于用户输入，一个`TextBlock`元素(名为`_result`)用于输出。 为此，我们需要处理`Button`的`Click`事件。为此，只需在设计器中双击该按钮。这将在`MainPage.xaml.cpp`文件中添加一个事件处理程序(以及相应的头文件和 XAML 中该按钮的`Click`属性)。Visual Studio 应自动打开`MainPage.xaml.cpp`。生成的事件处理程序如下所示：

```cpp
void CH01*HelloLive::MainPage::Button*Click_1(
...
```cpp
void MainPage::Button*Click*1(Object^ sender, RoutedEventArgs^ e)
{
}
```
...
![Creating your first Store application](img/5022*01*06.jpg)
...
![Application deployment](img/5022*01*07.jpg)
...
![Application deployment](img/5022*01*08.jpg)
...
![Where did int.ToString come from?](img/5022*01*09.jpg)
...
![Project structure](img/5022*01*10.jpg)
...
![Project structure](img/5022*01*11.jpg)
...
![Project structure](img/5022*01*12.jpg)
