...
public:
  ...
  void set*rating(int rating) { *rating = rating; }
...
public:
  ...
  void add*review(const book*review& review);
  size*t reviews*count() const { return _reviews.size(); }
  const book*review& get*review(size_t index) const { 
...
private:
  std::wstring _name;
  std::wstring _author;
  std::vector<book*review> *reviews;
};
```
...
为了包装与图书相关的类，我们将创建一个 Windows Runtime 组件项目(我们将其命名为`BookLibrary`)。 然后，我们将添加一个 C++/CX WinRT 类来包装`book`和`book*review`。 让我们从`book*review`包装器开始：
...
public:
  ...
  property int Rating {
    ...
    void set(int rating) { *review.set*rating(rating); }
  }
private:
  book*review *review;
};
...
String^ BookReview::Name::get() {
  return ref new String(*review.name().c*str());
}

String^ BookReview::Content::get() {
  return ref new String(*review.content().c*str());
}
```
...
public:
  ...
  property Platform::String^ Name {
    Platform::String^ get() { 
      return ref new Platform::String(*book.name().c*str()); 
    }
  }

  property Platform::String^ Author {
    Platform::String^ get() { 
      return ref new Platform::String(*book.author().c*str()); 
    }
  }
...
IVectorView<BookReview^>^ Book::Reviews::get() {
  if(_reviews == nullptr) {
    ...
    for(size*t i = 0; i < *book.reviews_count(); i++) {
      auto review = *book.get*review(i);
      reviews->Append(
        ref new BookReview(
...
![Consuming Windows Runtime Components](img/5022*06*01.jpg)
...
![Consuming Windows Runtime Components](img/5022*06*02.jpg)
...
![Consuming Windows Runtime Components](img/5022*06*03.jpg)
...
![Consuming Windows Runtime Components](img/5022*06*04.jpg)
...
![Other C++ library projects](img/5022*06*05.jpg)
...
![Building a control template](img/5022*06*06.jpg)
...
![Building a control template](img/5022*06*07.jpg)
...
![Building a control template](img/5022*06*08.jpg)
...
![Using the control's properties](img/5022*06*09.jpg)
...
![Handling state changes](img/5022*06*10.jpg)
...
![Handling state changes](img/5022*06*11.jpg)
...
![Handling state changes](img/5022*06*12.jpg)
...
![Creating a color picker user control](img/5022*06*13.jpg)
...
```cpp
DEFINE*DP*EX(Red, int, ColorPicker, 0, OnRGBChanged);
DEFINE*DP*EX(Green, int, ColorPicker, 0, OnRGBChanged);
DEFINE*DP*EX(Blue, int, ColorPicker, 0, OnRGBChanged);
```
...
void MainPage::OnColorChanged(Object^ sender, Color color) {
  ...
  *color->Text = ref new String(ss.str().c*str());
}
```

这会更改控件底部的`TextBlock`。 它是这样看待运行时的：

![Using the ColorPicker](img/5022*06*14.jpg)
...
![Creating a ColorPicker custom control](img/5022*06*15.jpg)
