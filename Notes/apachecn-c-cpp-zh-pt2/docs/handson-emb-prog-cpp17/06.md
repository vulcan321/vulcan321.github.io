...
# define  PWM_OUTPUT         2
# define  GPIO_CLOCK         3
# define  SOFT*PWM*OUTPUT          4
# define  SOFT*TONE*OUTPUT   5
# define  PWM*TONE*OUTPUT          6
```
...
# define  INT*EDGE*SETUP          0
# define  INT*EDGE*FALLING  1
# define  INT*EDGE*RISING         2
# define  INT*EDGE*BOTH           3
```
...
void WiringTimer::trigger(Poco::Timer &t) {
    if (triggerCnt == 0) {
          ...
          PIN0VAL.open("pin0val", std::ios*base::binary | std::ios*base::trunc);
          PIN0VAL.put(val);
          PIN0VAL.close();
          ...
          PIN7VAL.open("pin7val", std::ios*base::binary | std::ios*base::trunc);
          PIN7VAL.put(val);
          PIN7VAL.close();
...
//This value is then returned to the calling function.
 ...
 int wiringPiISR(int pin, int mode, void (*function)(void)) {
    ...
    if (pin == 0) { 
          ...
          Wiring::wt->isr*0*set = true;
    }
    else if (pin == 7) {
          Wiring::wt->isrcb_7 = function;
          Wiring::wt->isr*7*set = true;
    }

    if (Wiring::wt->isr*0*set && Wiring::wt->isr*7*set) {
          Wiring::wt->start();
    }
...
```cpp
static inline int i2c*smbus*access (int fd, char rw, uint8*t command, int size, union i2c*smbus_data *data) { 
   struct i2c*smbus*ioctl_data args; 
...
在这里，`i2c*smbus*data`类型是一个简单的联合，以支持返回值的各种大小 (执行读取操作时):

```cpp
union i2c*smbus*data { 
   uint8_t byte; 
   uint16_t word; 
   uint8*t block[I2C*SMBUS*BLOCK*MAX + 2]; 
}; 
```
...
```cpp
 ...
 PLATFORM_FLAGS = -D__PLATFORM_LINUX__ -D*LARGEFILE64*SOURCE -D **LINUX**
 STD*FLAGS = $(PLATFORM*FLAGS) -Og -g3 -Wall -c -fmessage-length=0 -ffunction-sections -fdata-sections -DPOCO*HAVE*GCC*ATOMICS -DPOCO*UTIL*NO*XMLCONFIGURATION -DPOCO*HAVE*FD_EPOLL
 STD*CFLAGS = $(STD*FLAGS)
 STD*CXXFLAGS = -std=c++ 11 $(STD*FLAGS)
 ...
 STD*LIBDIRS = $(STD*LDFLAGS)
 STD*LIBS = -ldl -lrt -lboost*system -lssl -lcrypto -lpthread
...
TS*SAFE = *$(shell date --date=@`git show -s --format=%ct` -u +%Y-%m-%dT%H%M%SZ) 
else 
TIMESTAMP = $(shell date -u +%Y-%m-%dT%H:%M:%SZ) 
TS*SAFE = *$(shell date -u +%Y-%m-%dT%H%M%SZ) 
endif 
```
...
CPP*OBJECTS := $(addprefix $(OBJROOT)/,$(CPP*SOURCES:.cpp=.o)) 
OBJECTS := $(CPP_OBJECTS) 
```
...
all: makedirs $(CPP*OBJECTS) $(C*OBJECTS) $(TOP)/build/bin/$(TARGET)/$(OUTPUT)*$(VERSION)*$(TARGET)$(TS_SAFE) 
...
```cpp
$(TOP)/build/bin/$(TARGET)/$(OUTPUT)*$(VERSION)*$(TARGET)$(TS_SAFE): $(OBJECTS) 
...
openssl: 
   ...
   cd openssl-1.0.2 && ./Configure --openssldir="$(TOP)/build/$(TARGET)/openssl" shared os/compiler:$(TOOLCHAIN*NAME):$(OPENSSL*PARAMS) && \ 
     $(MAKE) build_libs 
