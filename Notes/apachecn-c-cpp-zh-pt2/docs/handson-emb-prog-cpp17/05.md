...
保持室内植物存活是一项不小的壮举。本章中的示例项目将向您展示如何创建具有 Wi-Fi 功能的土壤湿度监测器，该监测器具有泵或类似设备的执行器选项，例如阀门和重力供水水箱。使用内置的 web 服务器，我们将能够使用其基于浏览器的 UI 来监控工厂运行状况和控制系统功能，或者使用其基于 HTTP 的 REST API 将其集成到更大的系统中。
...
除了测量土壤湿度水平外，我们还希望系统能够以设定的触发水平自动为植物浇水，并使我们能够监控此过程。这需要某种网络访问，最好是无线的，这样我们就不必比电源连接器更多地运行电缆。
...
这样，我们以后还可以编写一个后端服务，将许多这样的节点组合成一个具有中央控制和管理的连贯系统。这是我们实际上将在[第 9 章](09.html)，*示例-建筑物监控和控制*中详细介绍的内容。
...
对于这个项目，我们将为我们将在[第 9 章](09.html)，*示例-建筑物监控和控制*中使用的相同固件实现模块。因此，本章将仅涵盖此植物浇水模块独有的部分。
...
`esptool2`实用程序是官方 SDK 中的一组脚本的替代方案，该脚本将链接器输出转换为我们可以写入 esp8266 的 ROM 格式。当我们编译应用时，它是由 Sming 调用的。
...
最后一行将工具链的二进制文件添加到路径中，这是我们在调试 ESP8266 应用时需要的，如[第 7 章](07.html)，*测试资源受限平台*中所示。此时，我们可以使用 Sming 进行开发并创建可以写入 MCU 的 ROM 映像。
...
| `SPI*MODE`] | 这将设置在将固件映像闪烁到 SPI ROM 时使用的 SPI 模式。只有两条数据线 (`SD*D0`，`D1`) 或四条 (`SD_D0-3`)。并非所有 SPI rom 都连接了所有四条数据线。`qio`模式更快，但`dio`应该始终有效。 |
| `RBOOT_ENABLED`] | 当设置为 1 时，这将启用 rBoot 引导加载程序支持。我们希望启用此功能。 |
| `RBOOT*BIG*FLASH`] | 有 4 MB 的 ROM 可用，我们希望使用所有这些。也启用此功能。 |
| `RBOOT*TWO*ROMS`] | 如果我们希望在单个 1 MB ROM 芯片中放置两个固件映像，则可以使用此选项。这适用于某些 ESP8266 模块和衍生物。 |
| `SPI_SIZE`] | 在这里，我们设置 SPI ROM 芯片的尺寸，对于该项目，该尺寸应为 4M。 |
| `SPIFF_FILES`] | 包含将被写入 MCU 的 SPIFFS ROM 映像上的文件的文件夹的位置。 |
| `SPIFFS*SIZE`] | 要创建的 SPIFFS ROM 图像的大小。在这里，64 KB 是标准的，但是如果需要，在启用`RBOOT*BIG_FLASH`选项的情况下使用 4 MB ROM 时，我们最多可以使用 1 MB。 |
...
| `USE*MQTT*PASSWORD`] | 如果您希望使用用户名和密码连接到 MQTT 代理，请设置为 true。 |
...
其中，wi-fi、MQTT 和 OTA 设置是必不可少的，因为它们将允许应用连接到网络和 MQTT 代理，以及接收固件更新，而无需通过其串行接口刷新 MCU。
...
对于包含主要应用逻辑的`OtaCore`类，如果我们希望在核心类完成网络、MQTT 和其他功能的设置后执行任何进一步的逻辑，我们只会调用它的静态初始化函数，同时提供回调函数。
...
class OtaCore {
         ...
         static void connectFail(String ssid, uint8*t ssidLength, uint8*t *bssid,    uint8_t reason);
...
HardwareSerial OtaCore::Serial1(UART*ID*1); // UART 0 is 'Serial'.
...
String getFileContent(const String fileName) {
         file*t file = fileOpen(fileName.c*str(), eFO_ReadOnly);
...
bool OtaCore::init(onInitCallback cb) {
         ...
         Serial1.begin(SERIAL*BAUD*RATE); 
         Serial1.systemDebugOutput(true);
```

我们首先初始化 NodeMCU 中的两个 uart (串行接口)。尽管 ESP8266 中正式有两个 uart，但第二个仅由 TX 输出线 (默认为 GPIO 2) 组成。因此，我们希望为需要完整串行线的应用 (例如某些传感器) 保持第一个 UART 免费。
...
```cpp
         int slot = rboot*get*current_rom();
         u32_t offset;
         if (slot == 0) { offset = 0x100000; }
         else { offset = 0x300000; }
         spiffs*mount*manual(offset, 65536);
```
...
```cpp
          ...
          Serial1.printf("\r\nSDK: v%s\r\n", system*get*sdk_version());
     Serial1.printf("Free Heap: %d\r\n", system*get*free*heap*size());
...
```cpp
         mqtt = new MqttClient(MQTT*HOST, MQTT*PORT, onMqttReceived);
```
...
```cpp
         ...
         WifiStation.config(WIFI*SSID, WIFI*PWD);
         WifiStation.connect();
...
void OtaCore::otaUpdate() {
         OtaCore::log(LOG*INFO, "Updating firmware from URL: " + String(OTA*URL));

         if (otaUpdater) { delete otaUpdater; }
         otaUpdater = new rBootHttpUpdate();

         rboot*config bootconf = rboot*get_config();
...
```cpp
# ifdef ENABLE_SSL
         mqtt->connect(MAC, MQTT*USERNAME, MQTT*PWD, true);
         mqtt->addSslOptions(SSL*SERVER*VERIFY_LATER);
...
```cpp
# elif defined USE*MQTT*PASSWORD
          mqtt->connect(MAC, MQTT*USERNAME, MQTT*PWD);
```
...
```cpp
               ...
               mqtt->publish(MQTT*PREFIX"cc/response", MAC + ";"                                                         + String((const char*) &active*mods, 4));
         }
         else if (cmd == "version") {
...
bool OtaCore::claimPin(int pin) {
    ...
    bool OtaCore::claimPin(ESP8266_pins pin) {
          if (esp8266_pins & pin) {
                ...
                log(LOG*DEBUG, String("Current claimed pins: ") + String(esp8266*pins));
                return false;
          }
          ...
          log(LOG*DEBUG, String("Claimed pin configuration: ") + String(esp8266*pins));
...
enum ModuleIndex {
   MOD*IDX*TEMPERATURE_HUMIDITY = 0,
   MOD*IDX*CO2,
   MOD*IDX*JURA,
   MOD*IDX*JURATERM,
   MOD*IDX*MOTION,
...
bool BaseModule::newConfig(uint32 config) {
    ...
    uint32 new*config = config ^ active*mods;
    if (new_config == 0x0) {
        OtaCore::log(LOG_INFO, "New configuration was 0x0\. No 
        change.");
        return true; 
    }
    OtaCore::log(LOG*INFO, "New configuration: " + new*config);
    for (uint8 i = 0; i < 32; ++ i) {
        if (new_config & (1 << i)) {
...
int PlantModule::pin = PLANT*GPIO*PIN;
...
enum {
         PLANT*SOIL*MOISTURE = 0x01,
         PLANT*SET*TRIGGER = 0x02,
         PLANT_TRIGGER = 0x04
};
```
...
bool PlantModule::initialize() {
          BaseModule::registerModule(MOD*IDX*PLANT, PlantModule::start,                                                                                                                 PlantModule::shutdown);
}
```
...
void PlantModule::commandCallback(String message) {
         ...
         if (cmd == PLANT*SOIL*MOISTURE) {
               readSensor();
         }
         else if (cmd == PLANT*SET*TRIGGER) {               
...
void PlantModule::readSensor() {
    int16_t val = 0;
    val = analogRead(A0); // calls system*adc*read().

    String response = OtaCore::getLocation() + ";" + val;
    OtaCore::publish(MQTT*PREFIX"nsa/plant/moisture*raw", response);
```
...
完成应用的代码后，我们可以在项目的根文件夹中使用单个命令对其进行编译:
...
在监控 cc/config 主题以获取消息时，我们应该看到 ESP8266 通过发布其`MAC`来报告其存在。我们也可以通过将 USB 到 TTL 串行适配器连接到串行日志输出引脚 (NodeMCU 上的`D4`) 来获得这一点。通过查看串行控制台上的输出，我们将看到系统的 ip 地址和`MAC`。
