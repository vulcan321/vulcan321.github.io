...
client = soundFoo.new('YOUR*CLIENT*ID');
// Fetch track to stream.
track = client.get("/tracks/293")
// Get the tracks streaming URL.
stream*url = client.get(track.stream*url, true); 
...
# define MSG*RECORD*MAX*SILENCE*US 5000000 
...
void MainWindow::processBuffer(QAudioBuffer buffer) { 
    ...
    if (silence >= MSG*RECORD*MAX*SILENCE*US) { 
        silence = 0; 
        audioRecorder->stop(); 
    } 
} 
...
void VoiceInput::run() { 
    ...
    uint8 utt*started, in*speech; 
    uint32 k = 0; 
    char const* hyp; 

    static ps*decoder*t *ps; 
...
```cpp
    ...
    if (ps*set*kws(ps, "keyword_search", keyfile) != 0) { 
        return; 
    } 

    if (ps*set*search(ps, "keyword_search") != 0) { 
        return; 
    } 
    ...
    const char* gramfile = "grammar asr;\ 
            ...
            \ 
            <preposition> = FROM | TO;"; 
    ps*set*jsgf_string(ps, "jsgf", gramfile); 
```

接下来，我们设置了关键字-spotting 和 JSGF 语法文件，该文件将在音频样本的处理过程中使用。通过第一个`ps*set*search()`函数调用，我们开始关键字点搜索。以下循环将继续处理样本，直到检测到语音`computer`:
...
```cpp
    ...
    for (;;) { 
        ...
        ps*process*raw(ps, adbuf, k, FALSE, FALSE); 
        in*speech = ps*get*in*speech(ps); 

        if (in*speech && !utt*started) { 
            utt_started = TRUE; 
            E_INFO("Listening...\n"); 
        } 
```
...
```cpp
        if (!in*speech && utt*started) { 
            ps*end*utt(ps); 
            hyp = ps*get*hyp(ps, nullptr); 
            if (hyp != nullptr) { 
                // We have a hypothesis. 

                if (kws && strstr(hyp, "computer") != nullptr) { 
                    if (ps*set*search(ps, "jsgf") != 0) { 
