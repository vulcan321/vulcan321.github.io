...
QmlInterface::QmlInterface(QObject *parent) : QObject(parent) { 
    ...
    mediaPlayer.setMedia(QUrl("qrc:/music/tiltshifted*lost*neon_sun.mp3")); 
...
# if defined(Q*OS*IOS) || defined(Q*OS*ANDROID) || defined(Q*OS*QNX) || defined(Q*OS*WINRT) 
    return false; 
# else 
    bool isTouch = false; 
    ...
    bool isMobile = false; 
    if (qEnvironmentVariableIsSet("QT*QUICK*CONTROLS_MOBILE")) { 
        isMobile = true; 
    } 
...
```cpp
    components: [external*forward*renderer] 
