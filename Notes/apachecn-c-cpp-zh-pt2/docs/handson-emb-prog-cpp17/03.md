...
`rtc-ds1307.c`文件包含两个我们需要读取和设置 RTC 的函数，分别是: `ds1307*get*time()`和`ds1307*set*time()`。这些功能的基本功能与我们将在本章后面的 club room 监控示例中使用的功能非常相似，在该示例中，我们只是将 I2C 设备支持集成到我们的应用中。
...
这个特殊的板是 Seeed 工作室覆盆子皮中继板 1.0 版: [http://wiki.seeedstudio.com/覆盆子皮 * 继电器 * 板 1.0 版/](http://wiki.seeedstudio.com/Raspberry*Pi*Relay*Board*v1.0/)。它提供了我们需要的四个继电器，允许我们开关灯和开关高达 30 VDC (直流电) 或 250 VAC (交流电)。这允许人们连接几乎任何类型的照明和进一步的继电器和亲属。
...
*   它具有有效的 EEPROM，其中包含供应商信息，GPIO 映射和连接到 Raspberry Pi SBC 上的`ID*SC`和`ID*SD`I2C 总线引脚的设备信息
...
```cpp
Listener listener("ClubStatus", mqtt*host, mqtt*port, mqtt*user, mqtt*pass);
...
# include "listener.h"
 ...
 Listener::Listener(string clientId, string host, int port, string user, string pass) : mosquittopp(clientId.c_str()) {
          int keepalive = 60;
          username*pw*set(user.c*str(), pass.c*str());
...
```cpp
 ...
 void Listener::sendMessage(string topic, string &message) {
          publish(0, topic.c*str(), message.length(), message.c*str(), true);
 }
...
```cpp
 # define REG*INPUT*PORT0              0x00
 # define REG*INPUT*PORT1              0x01
 # define REG*OUTPUT*PORT0             0x02
 # define REG*OUTPUT*PORT1             0x03
 # define REG*POL*INV_PORT0            0x04
 # define REG*POL*INV_PORT1            0x05
 # define REG*CONF*PORT0               0x06
 # define REG*CONG*PORT1               0x07
 # define REG*OUT*DRV*STRENGTH*PORT0_L 0x40
 # define REG*OUT*DRV*STRENGTH*PORT0_H 0x41
 # define REG*OUT*DRV*STRENGTH*PORT1_L 0x42
 # define REG*OUT*DRV*STRENGTH*PORT1_H 0x43
 # define REG*INPUT*LATCH_PORT0        0x44
 # define REG*INPUT*LATCH_PORT1        0x45
 # define REG*PUD*EN_PORT0             0x46
 # define REG*PUD*EN_PORT1             0x47
 # define REG*PUD*SEL_PORT0            0x48
 # define REG*PUD*SEL_PORT1            0x49
 # define REG*INT*MASK_PORT0           0x4A
 # define REG*INT*MASK_PORT1           0x4B
 # define REG*INT*STATUS_PORT0         0x4C
 # define REG*INT*STATUS_PORT1         0x4D
 # define REG*OUTPUT*PORT_CONF         0x4F
```
...
if (Club::relayActive) {
    ...
    wiringPiI2CWriteReg8(i2cHandle, REG*CONF*PORT0, 0x00);
    wiringPiI2CWriteReg8(i2cHandle, REG*OUTPUT*PORT0, 0x00);
...
void ClubUpdater::writeRelayOutputs() {
    wiringPiI2CWriteReg8(i2cHandle, REG*OUTPUT*PORT0, regOut0);
...
bool Club::start(bool relayactive, uint8_t relayaddress, string topic) {
          ...
          wiringPiISR(0, INT*EDGE*BOTH, &lockISRCallback);
          wiringPiISR(7, INT*EDGE*BOTH, &statusISRCallback);
