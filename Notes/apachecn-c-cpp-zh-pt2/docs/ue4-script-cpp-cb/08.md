...
1.  创建一个名为`King`的新的`StaticMeshActor`类。在类头中添加以下内容:
    ...
    ```cpp
    DECLARE*DYNAMIC*MULTICAST*DELEGATE*OneParam(FOnKingDeathSignature, AKing*, DeadKing);
    ```
...
2.  我们使用`DECLARE*DYNAMIC*MULTICAST*DELEGATE*OneParam`宏声明动态多播委托。动态多播委托允许任意数量的对象订阅 (监听) 和取消订阅 (停止监听)，以便在代理广播时通知它们。
...
5.  我们之前使用的`DECLARE*DYNAMIC*MULTICAST_DELEGATE`宏只声明了一个类型; 它没有声明委托的实例，所以我们现在就这样做，引用我们之前在调用宏时提供的类型名称。
...
5.  实施`PostEditChangeProperty`:
    ...
    void APostEditChangePropertyActor::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
    {
      if (PropertyChangedEvent.Property != nullptr)
      {
        ...
        if (PropertyName == GET*MEMBER*NAME_CHECKED(APostEditChangePropertyActor, ShowStaticMesh))
        {
...
9.  接下来我们需要使用`GET*MEMBER*NAME_CHECKED`宏。宏需要许多参数。
10.  第一个是要检查的类的名称。
11.  第二个参数是检查类的属性。
...
9.  添加以下代码:
    ...
    IMPLEMENT*GAME*MODULE(FUE4CookbookEditorModule, UE4CookbookEditor)
    ```
...
22.  在`.cpp`文件中，我们包含模块的标头，然后使用`IMPLEMENT*GAME*MODULE`宏。
23.  `IMPLEMENT*GAME*MODULE`声明一个导出的 C 函数，`InitializeModule()`，它返回我们新的 module 类的一个实例。
...
1.  创建一个新的头文件，并插入以下类声明:
    ...
    class FCookbookCommands : public TCommands<FCookbookCommands>
    {
      public:
      FCookbookCommands()
      :TCommands<FCookbookCommands>(FName(TEXT("UE4*Cookbook")), FText::FromString("Cookbook Commands"), NAME*None, FEditorStyle::GetStyleSetName()) 
      {
      };
...
3.  在模块类中添加以下内容:
    ...
    void AddToolbarExtension(FToolBarBuilder &builder)
    {
      ...
      builder.AddToolBarButton(FCookbookCommands::Get().MyButton, NAME*None, FText::FromString("My Button"), FText::FromString("Click me to display a message"), IconBrush, NAME*None);
    };
    ```
...
13.  因此，即使我们不打算使用本地化，我们也需要用`# defines`封闭我们的`UI*COMMAND`调用，以便为`LOCTEXT*NAMESPACE`设置有效值。
...
1.  基于`FAssetTypeActions*Base`创建一个新类。您需要在头文件中包含`AssetTypeActions*Base.h`。
2.  重写类中的以下虚拟函数:
...
30.  创建新窗口后，我们将检索对主应用框架模块的引用。如果编辑器的顶层窗口存在并且有效，我们将新窗口实例添加为该顶层窗口的子级。
...
![Creating a new graph pin visualizer for Blueprint](img/00204.jpeg)

对于某些类型的资产，我们可能希望以与`FVector`等类支持以下方式相同的方式启用文字值的在线编辑:

![Creating a new graph pin visualizer for Blueprint](img/00205.jpeg)
...
3.  创建另一个名为`SGraphPinCustomAsset`的头文件:
    ...
    {
      SLATE*BEGIN*ARGS(SGraphPinCustomAsset) {}
      SLATE*END*ARGS()
...
10.  Create a new **Function** inside the Level Blueprint by clicking on the plus symbol beside **Functions** within the **My Blueprint** panel:
    ...
    ![How to do it...](img/00206.jpeg)

11.  Add an input parameter.

    ![How to do it...](img/00207.jpeg)

12.  Set its type to `MyCustomAsset` (**Reference**):

    ![How to do it...](img/00208.jpeg)
...
![Inspecting types with custom Details panels](img/00210.jpeg)
...
1.  Details 自定义是通过`IDetailCustomization`接口执行的，开发人员可以在定义自定义某个类的资产显示方式的类时继承该接口。
