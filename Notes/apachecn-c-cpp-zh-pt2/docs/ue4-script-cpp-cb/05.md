...
1.  将新的委托声明添加到`GameMode`:
    ...
    ```cpp
    DECLARE*DELEGATE*OneParam(FParamDelegateSignature, FLinearColor)
    ```
...
1.  我们新的委托签名使用稍微不同的宏进行声明。注意`DECLARE*DELEGATE*OneParam`末尾的`_OneParam`后缀。如您所料，我们还需要指定参数的类型。
...
1.  当我们将函数绑定到委托时，我们会指定一些额外的数据 (在本例中，值为`false`的布尔值)。您可以以这种方式传递多达四个 “payload” 变量。在您使用的`DECLARE*DELEGATE**`宏中声明的任何参数之后，它们将应用于您的函数。
...
1.  在`GameMode`头添加新的委托声明:
    ...
    ```cpp
    DECLARE*MULTICAST*DELEGATE(FMulticastDelegateSignature)
    ```
...
2.  在标头中添加多播委托声明:
    ...
    ```cpp
    DECLARE*MULTICAST*DELEGATE_TwoParams(FOnTimeChangedSignature, int32, int32)
    ```
