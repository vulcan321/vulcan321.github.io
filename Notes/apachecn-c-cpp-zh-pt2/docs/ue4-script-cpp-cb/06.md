...
***   打开您的`BP*GameMode`蓝图，并选择您的 Blueprinted`BP*Warrior`类作为默认`Pawn`类。*   要设置键盘的输入驱动播放器，请打开**设置** | **项目设置** | **输入**。在以下步骤中，我们将完成驱动玩家在游戏中前进的过程:
...
1.  在**设置** | **项目设置** | **输入**中，为您的热键事件定义一个新的动作映射，例如`HotKey*UIButton*Spell`。
