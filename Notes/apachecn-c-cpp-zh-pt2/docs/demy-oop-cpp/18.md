...
在前面提到的`main()`函数中，我们首先实例化三个`Person`实例。然后，我们实例化一个`CitizenDataBase`，以提供将我们的`Person`实例写入数据库或从数据库读取的封装访问。我们的`CitizenDataBase`构造函数的方法调用外部函数`db*open()`来打开数据库。同样，析构函数调用`db*close()`。不出所料，我们针对`Read()`和`Write()`的`CitizenDataBase`方法将分别调用外部函数`db*read()`或`db*write()`。
