...
![Figure 6.1 – Illustration of the connection mechanism between the sender and receiver ](img/Figure*6.1*B16231.jpg)
...
您可以通过对`QObject`派生类使用`qobject*cast()`来执行类型强制转换。 `qobject*cast()`函数的作用类似于标准 C++ `dynamic*cast()`。 优点是它不需要**运行时类型标识**(**RTTI**)，并且可以跨动态库边界工作。 您可以从`QObject`派生一个类，但是如果不添加`Q*OBJECT`宏，那么信号和槽以及其他元对象系统特性将不可用。 没有元代码的`QObject`派生类等同于包含元对象代码的最接近的祖先类。 还有一个更轻量级的`Q*OBJECT`宏版本，称为`Q*GADGET`，可用于利用`QMetaObject`提供的一些功能。 使用`Q_GADGET`的类没有信号或槽。
...
在本节中，我们将查看 Qt6 中`moc`生成的代码。 当您构建前面的信号和槽示例时，您将在 build 目录下看到生成的文件：`moc*myclass.cpp`和`moc*predefs.h`。 让我们使用文本编辑器打开`moc_myclass.cpp`文件：
...
# if !defined(Q*MOC*OUTPUT_REVISION)
# error "The header file 'myclass.h' doesn't include 
        <QObject>."
# elif Q*MOC*OUTPUT_REVISION != 68
...
```cpp
static const uint qt*meta*data_MyClass[] = {
 // content:
       9,       // revision
       0,       // classname
       0,    0, // classinfo
...
```cpp
void *MyClass::qt*metacast(const char **clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, 
                qt*meta*stringdata_MyClass.stringdata0))
        return static_cast<void*>(this);
    return QObject::qt*metacast(*clname);
}
```

Qt 的 Meta-Object 系统使用`qt*metacall()`函数来访问特定`QObject`对象的元信息。 当我们发出信号时，会调用`qt*metacall()`，然后再调用实际信号函数：

```cpp
int MyClass::qt*metacall(QMetaObject::Call *c, int *id, void ***a)
{
    *id = QObject::qt*metacall(*c, *id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 2)
            qt*static*metacall(this, *c, *id, _a);
        _id -= 2;
    } else if (_c == QMetaObject::
               RegisterMethodArgumentMetaType) {
        if (_id < 2)
            *reinterpret*cast<QMetaType *>(*a[0]) = 
                                           QMetaType();
        _id -= 2;
    }
    return _id;
...
通过这样做，您可以探索完整的生成代码并进一步查看符号。 现在，让我们看一下调用插槽的`moc`生成的代码。 插槽由其在`qt*static*metacall`函数中的索引调用，如下所示：

```cpp
void MyClass::qt*static*metacall(QObject *_o, 
    QMetaObject::Call *c, int *id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto **t = static*cast<MyClass *>(_o);
        (void)_t;
        switch (_id) {
        ...
        int *result = reinterpret*cast<int *>(*a[0]);
        {
            using _t = void (MyClass::*)();
            if (*reinterpret*cast<*t *>(_a[1]) == 
                static*cast<*t>(&MyClass::signalName)) {
                *result = 0;
                return;
            }
        }
...
```cpp
     Q*PROPERTY(QString text MEMBER m*text NOTIFY 
                textChanged)
signals:
...
![Figure 6.2 – Creating signals and slots connections using Qt Designer ](img/Figure*6.2*B16231.jpg)
...
在 C++ 中，要与 QML 层交互，可以使用信号、槽和`Q*INVOKABLE`函数。 也可以使用`Q*PROPERTY`宏来创建特性。 要响应来自对象的信号，可以使用`Connections`QML 类型。 当 C++ 文件中的属性发生更改时，`Q_PROPERTY`会自动更新这些值。 如果该属性与任何 QML 属性有绑定，它将自动更新 QML 内的属性值。 在这种情况下，信号插槽机制是自动建立的。
...
在前面的代码中，我们声明了一个基于 Q*PROPERTY 的通知。 当发出`counterChanged()`信号时，我们可以得到新的`counter`值。 但是，我们将`receiveFromQml()`函数用作`Q*INVOKABLE`函数，以便可以在 QML 文档中直接调用它。 我们发出的是`sendToQml()`，它在`main.qml`内部处理：
...
![Figure 6.3 – The mechanism that was used in this example ](img/Figure*6.3*B16231.jpg)
...
![Figure 6.4 – Illustration of an event-driven application using the event loop ](img/Figure*6.4*B16231.jpg)
...
[https：//wiki.qt.io/Threads*Events*QObjects](https://wiki.qt.io/Threads*Events*QObjects).
...
![Figure 6.5 – Illustration of the event filter mechanism ](img/Figure*6.5*B16231.jpg)
