...
![Calculating the level bounds](img/B04920*04*01.jpg)
...
for (int i = 0; i < GRID_WIDTH; ++ i)
{
    for (int j = 0; j < GRID_HEIGHT; ++ j)
    {
        if (m*grid[i][j].type == TILE::FLOOR || m*grid[i][j].type == TILE::FLOOR_ALT)
        { 
            m_grid[i][j].markAsSpawnable();
        }
    }
...
![Checking the underlying game grid](img/B04920*04*02.jpg)
...
sf::Vector2f Level::GetActualTileLocation(int columnIndex, int rowIndex)
{
    ...
    location.x = m*origin.x + (columnIndex * TILE*SIZE) + (TILE_SIZE / 2);
    location.y = m*origin.y + (rowIndex * TILE*SIZE) + (TILE_SIZE / 2);
...
![Spawning items at a random location](img/B04920*04*03.jpg)
...
![The complete spawn functions](img/B04920*04*04.jpg)
...
```cpp
static int const MAX*ITEM*SPAWN_COUNT = 50;
```
...
{
    // Spawn items.
    for (int i = 0; i < MAX*ITEM*SPAWN_COUNT; i++)
    {
        if (std::rand() % 2)
        {
...
```cpp
  static int const MAX*ENEMY*SPAWN_COUNT = 20;
```
...
void Game::SpawnEnemy(ENEMY enemyType, sf::Vector2f position)
{
    ...
    m*enemies.push*back(std::move(enemy));
}
```
...
void Game::PopulateLevel()
{
    ...
    // Spawn enemies.
    for (int i = 0; i < MAX*ENEMY*SPAWN_COUNT; i++)
    {
        if (std::rand() % 2)
        {
            SpawnEnemy(static*cast<ENEMY>(std::rand() % static*cast<int>(ENEMY::COUNT)));
        }
    }
}
```
...
![Randomly spawning enemies](img/B04920*04*05.jpg)
...
m*level.AddTile("../resources/tiles/spr*tile*floor*alt.png", TILE::FLOOR_ALT);
```
...
![Implementing the SpawnRandomTiles function](img/B04920*04*06.jpg)
