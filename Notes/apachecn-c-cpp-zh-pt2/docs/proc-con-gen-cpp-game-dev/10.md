...
![Convoluted inheritance structures](img/B04920*10*02.jpg)
...
![Circular dependencies](img/B04920*10*03.jpg)
...
![Avoiding complex inheritance structures](img/B04920*10*01.jpg)
 ...
 */
std::vector<std::shared*ptr<Component>> m*components;
```

现在是时候使用实际的`AttachComponent`方法了。我们可以采取一种天真的方法，只需将新组件附加到`generic`容器中。这里的问题是，我们可以添加相同组件类型的倍数，这不是我们想要的。在将组件添加到集合中之前，我们将首先检查相同类型的组件是否已经存在，为此，我们将使用`std::dynamic*pointer*cast`函数。
...
std::shared_ptr<T> AttachComponent()
{
    ...
    std::shared*ptr<T> newComponent = std::make*shared<T>();

    // Check that we don't already have a component of this type.
    for (std::shared*ptr<Component>& exisitingComponent : m*components)
    {
        if (std::dynamic*pointer*cast<T>(exisitingComponent))
        {
            // If we do replace it.
    ...
    m*components.push*back(newComponent);
...
std::shared_ptr<T> GetComponent()
{
    ...
    for (std::shared*ptr<Component> exisitingComponent : m*components)
    {
        if (std::dynamic*pointer*cast<T>(exisitingComponent))
        {
            return std::dynamic*pointer*cast<T>(exisitingComponent);
        }
    }
...
std::shared*ptr<Component> component = m*player.GetComponent<Component>();
```
...
![Updating the game code](img/B04920*10*04.jpg)
...
![Updating the game code](img/B04920*10*05.jpg)
...
```cpp
    ...
    GetComponent<AudioComponent>()->SetSoundBuffer("../resources/sounds/snd*player*hit.wav");
}
```
