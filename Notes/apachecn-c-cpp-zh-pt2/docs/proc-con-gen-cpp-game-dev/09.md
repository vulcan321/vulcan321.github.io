...
![An overview of dungeon generation overview](img/B04920*09*01.jpg)
...
![Generating rooms](img/B04920*09*02.jpg)
...
![Procedurally generating a dungeon](img/B04920*09*05.jpg)
...
case GAME_STATE::PLAYING:
{
  ...
  //m*window.setView(m*views[static_cast<int>(VIEW::MAIN)]);
```
...
![Changing how we view the maze](img/B04920*09*06.jpg)
...
if (playerTile.type == TILE::WALL*DOOR*UNLOCKED)
{
...
```cpp
// Load the level.
//m*level.LoadLevelFromFile("../resources/data/level*data.txt");
...
![Preparing before the generation of a maze](img/B04920*09*07.jpg)
...
void Level::GenerateLevel()
{
    ...
    for (int i = 0; i < GRID_WIDTH; ++ i)
    {
        for (int j = 0; j < GRID_HEIGHT; ++ j)
        {
            ...
            {
                m*grid[i][j].type = TILE::WALL*TOP;
                m*grid[i][j].sprite.setTexture(TextureManager::GetTexture(m*textureIDs[static*cast<int>(TILE::WALL*TOP)]));
            }
            // Set the position.
            m*grid[i][j].sprite.setPosition(m*origin.x + (TILE*SIZE * i), m*origin.y + (TILE_SIZE * j));
        }
    }
}
```
...
![Preparing before the generation of a maze](img/B04920*09*08.jpg)
...
```cpp
  ...
  tile->sprite.setTexture(TextureManager::GetTexture(m*textureIDs[static*cast<int>(TILE::FLOOR)]));
  ...
  wall->sprite.setTexture(TextureManager::GetTexture(m*textureIDs[static*cast<int>(TILE::FLOOR)]));
...
![Carving passages](img/B04920*09*09.jpg)
...
if (TileIsValid(newI, newY))
{
  ...
  if ((newI != 0) && (newI != (GRID*WIDTH - 1)) && (newY != 0) && (newY != (GRID*HEIGHT - 1)))
  {
    m_grid[newI][newY].type = TILE::FLOOR;
    m*grid[newI][newY].sprite.setTexture(TextureManager::GetTexture(m*textureIDs[static_cast<int>(TILE::FLOOR)]));
  }
}}}}}
```
...
![Adding rooms](img/B04920*09*10.jpg)
...
![Bitwise tile maps](img/B04920*09*11.jpg)
...
```cpp
// Set the new type.
m*grid[i][j].type = static*cast<TILE>(value);
m*grid[i][j].sprite.setTexture(TextureManager::GetTexture(m*textureIDs[value]));
}}}}
```
...
![Calculating tile textures](img/B04920*09*15.jpg)
...
我们可以用这些来跟踪我们已经产生了多少房间，以及我们应该什么时候改变效果。首先，我们必须跟踪我们在哪个楼层和房间。在`Level::GenerateLevel`函数的末尾，我们将从递增`m*roomNumber`变量开始。当它是`5`时，我们可以增加`m*floorNumber`并生成新的颜色叠加; 不要忘记重置房间计数器:
...
![Creating unique floor themes](img/B04920*09*16.jpg)
...
void Level::GenerateEntryExit()
{
  ...
  while (startI == -1)
  {
    ...
    if (m*grid[index][GRID*HEIGHT - 1].type == TILE::WALL_TOP)
    {
      startI = index;
    }
  }
  ...
  while (endI == -1)
  {
    ...
    if (m*grid[index][0].type == TILE::WALL*TOP)
    {
      endI = index;
    }
}
...
现在，我们已经确定了开始节点和结束节点，剩下的就是将节点设置为正确的图块类型。入口节点需要设置为`TILE::WALL*ENTRANCE`，出口节点必须设置为`TILE::WALL*DOOR_LOCKED`，如下所示:

```cpp
  // Set the tile textures for the entrance and exit tiles.
  SetTile(startI, GRID*HEIGHT - 1, TILE::WALL*ENTRANCE);
  SetTile(endI, 0, TILE::WALL*DOOR*LOCKED);
}
```
...
```cpp
 ...
 m*spawnLocation = GetActualTileLocation(startI, GRID*HEIGHT - 2);
}
```
...
void Game::GenerateLevel()
{
 ...
 m*player.SetPosition(m*level.SpawnLocation());
}
```

是时候测试代码了。现在，当我们运行游戏时，我们不仅应该看到一个看起来很棒的迷宫，而且还应该看到一个底部的入口，我们的玩家就在它的正上方，以及一个在关卡顶部的出口:

![Setting a player's spawn location](img/B04920*09*17.jpg)
