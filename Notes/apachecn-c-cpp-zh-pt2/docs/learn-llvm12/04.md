...
`tinylang`的项目布局遵循我们在[*第 2 章*](02.html# *idTextAnchor032)，*浏览 LLVM 源*中介绍的方法。 每个组件的源代码位于`lib`目录的子目录中，而头文件位于`include/tinylang`目录的子目录中。 该子目录以组件命名。 在[*第 2 章*](02.html# *idTextAnchor032)，*浏览 LLVM 源*中，我们只创建了`Basic`组件。

从上一章中，我们知道我们需要实现一个词法分析器、一个解析器、一个 AST 和一个语义分析器。 每个组件都有自己的组件，称为`Lexer`、`Parser`、`AST`和`Sema`。 上一章中使用的目录布局如下所示：

![Figure 4.1 – The directory layout of the tinylang project ](img/Figure*4.1*B15647.jpg)
...
# endif
DIAG(err*sym*declared, Error, "symbol {0} already declared")
# undef DIAG
```
...
```cpp
# ifndef TINYLANG*BASIC*DIAGNOSTIC_H
# define TINYLANG*BASIC*DIAGNOSTIC_H
...
```cpp
# ifndef TINYLANG*BASIC*TOKENKINDS_H
# define TINYLANG*BASIC*TOKENKINDS_H
namespace tinylang {
namespace tok {
...
bool Parser::parseIfStatement() {
  auto _errorhandler = [this] {
    return SkipUntil(tok::semi, tok::kw*ELSE, tok::kw*END);
  };
...
{space}+
{digit}+      return
                   tinylang::Parser::token::T*integer*literal;
```
...
```cpp
# ifndef TINYLANG*SEMA*SCOPE_H
# define TINYLANG*SEMA*SCOPE_H
# include "tinylang/Basic/LLVM.h"
...
class Decl {
public:
  enum DeclKind { DK*Module, DK*Const, DK_Type,
                  DK*Var, DK*Param, DK_Proc };
private:
  const DeclKind Kind;
public:
  ...
  if (TypeDeclaration *Ty = dyn_cast<TypeDeclaration>(D)) {
    for (auto I = Ids.begin(), E = Ids.end(); I != E; ++ I) {
      ...
      if (CurrentScope->insert(Decl))
        ...
        Diags.report(Loc, diag::err*symbold*declared, Name);
    }
  } else if (!Ids.empty()) {
    SMLoc Loc = Ids.front().first;
    Diags.report(Loc, diag::err*vardecl*requires_type);
  }
}
```
...
bool Parser::parseProcedureDeclaration(/* … */) {
  /* … */
  if (consume(tok::kw*PROCEDURE)) return *errorhandler();
...
void Sema::actOnProcedureDeclaration(
  ...
  if (Name != ProcDecl->getName()) {
    Diags.report(Loc, diag::err*proc*identifier*not*equal);
    Diags.report(ProcDecl->getLocation(),
                 diag::note*proc*identifier_declaration);
  }
  ProcDecl->setDecls(Decls);
  ProcDecl->setStmts(Stmts);
  auto RetTypeDecl =
      dyn*cast*or_null<TypeDeclaration>(RetType);
  if (!RetTypeDecl && RetType)
    Diags.report(Loc, diag::err*returntype*must*be*type,
                 Name);
  else
