...
before_script:
  - export CONAN*USER*HOME="$CI*PROJECT*DIR"
...
build:
  ...
  script:
    ...
    - cmake -DBUILD*TESTING=1 -DCMAKE*BUILD_TYPE=Release ../ch08/customer
    - cmake --build .
```
...
remote:   https://gitlab.com/hosacpp/continuous-integration/merge*requests/new?merge*request%5Bsource_branch%5D=fix-ci-cd
remote:                         
```
...
# variable
resource "aws*key*pair" "deployer" {
  key*name = var.aws*key_name

  public*key = file(var.public*key_path)
}
...
resource "aws_instance" "project" {
  ...
  key*name = aws*key*pair.deployer.key*name
