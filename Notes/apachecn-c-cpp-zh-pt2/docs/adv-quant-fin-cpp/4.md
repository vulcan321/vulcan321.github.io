...
double SampleBoxMuller()
{
  ...
  do
  {
    x = 2.0*rand()/static*cast<double>(RAND*MAX)-1;
    y = 2.0*rand()/static*cast<double>(RAND*MAX)-1;
    xysquare = x*x + y*y;
  }
  while
  ( xysquare >= 1.0);
...
在前面的等式中，![Advanced example – equity basket](img/00188.jpeg)是两个独立于高斯分布的样本，而![Advanced example – equity basket](img/00189.jpeg)是合并了相关性![Advanced example – equity basket](img/00190.jpeg)影响的两个相关样本。 ε*1 和ε*2 是正态变量，因为它们具有一元方差，其乘积![Advanced example – equity basket](img/00191.jpeg)的期望值等于![Advanced example – equity basket](img/00190.jpeg)。
