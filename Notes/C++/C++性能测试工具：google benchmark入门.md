...
```bash
git clone https://github.com/google/benchmark.gitgit clone https://github.com/google/googletest.git benchmark/googletestmkdir build && cd buildcmake -DCMAKE*BUILD*TYPE=RELEASE ../benchmarkmake -j4# 如果想全局安装就接着运行下面的命令sudo make install
```
...
static void bench*array*operator(benchmark::State& state)
{
...
BENCHMARK(bench*array*operator);
 
// 使用at()读取元素，依次存入1-6的平方
static void bench*array*at(benchmark::State& state)
{
    std::array<int, len> arr;
    constexpr int i = 1;
    for (auto _: state) {
...
BENCHMARK(bench*array*at);
 
// std::get<>(array)是一个constexpr function，它会返回容器内元素的引用，并在编译期检查数组的索引是否正确
static void bench*array*get(benchmark::State& state)
{
    std::array<int, len> arr;
    constexpr int i = 1;
    for (auto _: state) {
...
// ring buffer的测试
static void bench*array*ring*insert*int_10(benchmark::State& state)
{
...
BENCHMARK(bench*array*ring*insert*int_10);
 
// linked list的测试
static void bench*linked*queue*insert*int_10(benchmark::State &state)
{
...
BENCHMARK(bench*linked*queue*insert*int_10);
...
```c++
static void bench*array*ring*insert*int(benchmark::State& state)
{
...
BENCHMARK(bench*array*ring*insert*int)->Arg(10);
```
...
BENCHMARK(bench*array*ring*insert*int)->Args({10, 10});
...
BENCHMARK(bench*array*ring*insert*int)->Arg(10);
BENCHMARK(bench*array*ring*insert*int)->Arg(100);
BENCHMARK(bench*array*ring*insert*int)->Arg(1000);
```
...
```c++
BENCHMARK(bench*array*ring*insert*int)->Arg(10)->Arg(100)->Arg(1000);
```
...
```c++
BENCHMAEK(func)->Range(int64*t start, int64*t limit);
```

start表示参数范围起始的值，limit表示范围结束的值，Range所作用于的是一个\*闭区间\*。

但是如果我们这样改写代码，是会得到一个错误的测试结果：

```c++
BENCHMARK(bench*array*ring*insert*int)->Range(10, 1000);
```
...
```c++
BENCHMARK(bench*array*ring*insert*int)->RangeMultiplier(10)->Range(10, 1000);
```
...
BENCHMARK(bench*array*ring*insert*int)->RangeMultiplier(10)->Apply(custom_args);
