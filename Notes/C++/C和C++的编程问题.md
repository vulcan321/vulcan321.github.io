...
```c++
std::string Read*Image(const std::string &*csImagePath)
{
    std::ifstream file(*csImagePath.c*str(), std::ios::in);
    int iBufSize = 0;
...
```text
/MD    使此应用程序使用特定于多线程和 DLL 的运行库版本。 定义 *MT 和 *DLL，并使编译器将库名 MSVCRT.lib 放入 .obj 文件中。
    用此选项编译的应用程序静态链接到 MSVCRT.lib。 此库提供使链接器能够解析外部引用的代码的层。 实际工作代码包含在 
    MSVCR versionnumber.DLL，该代码必须可运行时提供给与 MSVCRT.lib 链接的应用程序。


/MDd    定义 *DEBUG、*MT 和 _DLL，并使此应用程序使用特定于多线程和 DLL 的调试版本的运行库。 它还会让编译器将库 
...
/MTd    定义 *DEBUG 和 *MT。 此选项还会让编译器将库名称 LIBCMTD.lib 放置到 .obj 文件中，以便链接器将使用 LIBCMTD.lib 来            解析外部符号。
...
// C++14 起的写法
static int g_state = 1;
filters.emplace*back([g*state = g_state](int value) {
    return value % g_state;
});
```
...
```c
SDL*Event user*event;

user*event.type = SDL*USEREVENT;
user_event.user.code = 2;
user_event.user.data1 = NULL;
user_event.user.data2 = NULL;
SDL*PushEvent(&user*event);
```

​    可以这样做的原因是因为 **SDL*Event** 其他事件结构体都统一的将第一个成员声明为 **Uint32 type** ,  这样直接访问 **SDL*Event.type** 就可以访问到对应事件结构体的 **type** 成员。其他的事件结构体例如：
...
{
    Uint32 type;        /**< ::SDL*KEYDOWN or ::SDL*KEYUP */
    Uint32 timestamp;   /**< In milliseconds, populated using SDL_GetTicks() */
    Uint32 windowID;    /**< The window with keyboard focus, if any */
    Uint8 state;        /**< ::SDL*PRESSED or ::SDL*RELEASED */
...
typedef struct SDL_MouseMotionEvent
{
    ...
    Uint32 which;       /**< The mouse instance id, or SDL*TOUCH*MOUSEID */
...
```cmake
set*property(TARGET d3d9*render PROPERTY MSVC*RUNTIME*LIBRARY "MultiThreaded$<$<CONFIG:Debug>:Debug>")
```
...
```cmake
if (MSVC)
    add*compile*options("$<$<C*COMPILER*ID:MSVC>:/source-charset:utf-8>")
    add*compile*options("$<$<CXX*COMPILER*ID:MSVC>:/source-charset:utf-8>")
endif()
```
...
```cpp
AttachConsole(ATTACH*PARENT*PROCESS); // 将当前程序附着到父进程上，因为是从控制台启动的，所以当前程序的父进程就是那个控制台。 
