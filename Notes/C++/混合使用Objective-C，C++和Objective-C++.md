...
作者接着提到了另一个方法，显示分配一块内存，然后在它的基础上调用new来创建对象。首先声明char wrapped\*mem\[sizeof(CppObject)\]; 再使用wrapped = new(wrapped\*mem) CppObject();创建了实例wrapped。释放时if (wrapped) wrapped->~CppObject();  这样虽然可行，但不建议使用。
...
# else
# define OBJC*CLASS(name) typedef struct objc*object name
# endif
```
