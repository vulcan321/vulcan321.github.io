...
high*resolution*clock表示系统可用的最高精度的时钟，实际上就是system*clock或者steady*clock其中一种的定义，官方没有说明具体是哪个，不同系统可能不一样，看gcc chrono源码中high*resolution*clock是steady_clock的typedef。
...
- find*if*not：找到第一个不符合要求的元素迭代器，和find_if相反
- copy_if：复制满足条件的元素
...
- is*sorted、is*sorted_until：返回容器内元素是否已经排好序。
