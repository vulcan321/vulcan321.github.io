...
```
Copy Highlighter-hljs<location-of-ifw>\binarycreator.exe --offline-only -t <location-of-ifw>\installerbase.exe -p <package*directory> -c <config*directory>\<config*file> <installer*name>
...
```
Copy Highlighter-hljsrepogen.exe -p <package*directory> <repository*directory>
...
```
Copy Highlighter-hljs<location-of-ifw>\binarycreator.exe -t <location-of-ifw>\installerbase.exe -p <package*directory> -c <config*directory>\<config*file> -e <packages> <installer*name>
...
提供更新的最简单方法是重新创建资源库，并上传到Web服务器。 有关更多信息，请参阅[创建存储库 Creating Repositories](https://www.cnblogs.com/oloroso/p/6775243.html# 5*2*1)
